...
void write_audio(ModWinmmState *state, void *memory_block)
{
    ...
    if ( memory_block )
        ...
        new_block = calloc(1024, sizeof(float));

    state->process_func(NULL, left, right, 512, 44100, state->instance_data);
    ...
    wave_header->dwBufferLength = 1024*sizeof(float);
...
void modwinmm_activate(void *state)
{
    ...
    mw_state->midi_in = calloc(numdevs + 1, sizeof(HMIDIIN));

    /* MIDI In */
    for ( midi_dev = 0; midi_dev < numdevs; midi_dev++ )
    {
        MIDIINCAPS midi_in_caps;

        if ( midiInGetDevCaps(midi_dev, &midi_in_caps, sizeof(MIDIINCAPS)) == 0 )
            printf("MIDI device: %s.\n", midi_in_caps.szPname);

        midiInOpen(&mw_state->midi_in[midi_dev], midi_dev,
                   (DWORD_PTR) midi_func, (DWORD_PTR) mw_state,
                   CALLBACK_FUNCTION);
        midiInStart(mw_state->midi_in[midi_dev]);
    }
...
void modwinmm_deactivate(void *state)
{
    ...
    for ( i = 0; mw_state->midi_in[i]; i++ )
    {
        midiInReset(mw_state->midi_in[i]);
        midiInClose(mw_state->midi_in[i]);
    }

    free(mw_state->midi_in);
}
